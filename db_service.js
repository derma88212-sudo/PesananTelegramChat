 ...
 * Supports seamless hybrid operation between Supabase, Google Cloud Firestore, 
 * and MultiDB Local Store / Redis.
 * 
 * Fully production-ready with real database connections across all engines.
 */
...
class DatabaseService {
  ...
  async _runSeeding() {
    // 1. Seed Firestore & Local defaults
    try {
      // 1.1 Root Admin in Firestore & Local
      const rootAdminPayload = {
        admin_id: 'root_admin',
      ...
      if (!adminDoc.exists()) {
        ...
        await setDoc(doc(this.db, 'admins', 'root_admin'), rootAdminPayload);
      }
      saveLocalAdmin(rootAdminPayload);
    ...
    if (this.hasSupabase) {
      try {
        ...
        console.warn('[Supabase] Seeding note:', sbErr.message);
      }
    }
  ...
  async verifyAdminLogin(username, password) {
    ...
    const locals = getLocalAdmins();
    const localMatched = locals.find(a => (a.username?.toLowerCase() === cleanUser || a.admin_id === username) && a.password_hash === cleanPass);
    if (localMatched) {
      return {
        admin_id: localMatched.admin_id,
        username: localMatched.username,
        role: localMatched.role || 'admin'
      };
    }
  ...
  async incrementChannelCounter(channelId, field = 'clicks_count', amount = 1) {
    ...
    try {
      ...
      const target = channels.find(c => c.channel_id === channelId || c.id === channelId);
  ...
  // --- Referral Links (Tautan Promosi Khusus) ---
  async getReferralLinks(channelId = null) {
    let supabaseQuery = null;
    if (this.hasSupabase) {
      supabaseQuery = this.supabase.from('referral_links').select('*').order('created_at', { ascending: false });
      if (channelId) supabaseQuery = supabaseQuery.eq('channel_id', channelId);
    }
    return this.queryWithFallback(supabaseQuery, async () => {
      try {
        let q = collection(this.db, 'referral_links');
        if (channelId) {
          q = query(q, where('channel_id', '==', channelId));
        }
        const snap = await getDocs(q);
        if (!snap.empty) {
          return snap.docs.map(d => d.data()).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      ...
      try {
        const locals = getLocalReferralLinks();
        if (channelId) return locals.filter(r => r.channel_id === channelId);
        return locals;
      } catch (e) {
        return [];
      }
  ...
  async getReferralLink(referralId) {
    if (!referralId) return null;
    if (this.hasSupabase) {
      try {
        const { data, error } = await this.supabase.from('referral_links').select('*').eq('referral_id', referralId).single();
        if (!error && data) return data;
      } catch (e) {}
    }
    try {
      const snap = await getDoc(doc(this.db, 'referral_links', referralId));
      if (snap.exists()) return snap.data();
    } catch (e) {}

    try {
      const locals = getLocalReferralLinks();
      return locals.find(r => (r.referral_id || r.id) === referralId) || null;
    } catch (e) {
      return null;
    }
  }

  async getReferralLinkByCode(code) {
    if (!code) return null;
    const clean = String(code).trim().toUpperCase();
    if (this.hasSupabase) {
      try {
        const { data, error } = await this.supabase.from('referral_links').select('*').eq('referral_code', clean).single();
        if (!error && data) return data;
      } catch (e) {}
    }
    try {
      const q = query(collection(this.db, 'referral_links'), where('referral_code', '==', clean));
      const snap = await getDocs(q);
    ...
    try {
      const locals = getLocalReferralLinks();
      return locals.find(r => String(r.referral_code || '').toUpperCase() === clean) || null;
    } catch (e) {
      return null;
    }
  }

  async saveReferralLink(referral) {
    const referralId = referral.referral_id || `ref_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
    const payload = {
      ...referral,
      referral_id: referralId,
      referral_code: String(referral.referral_code || '').trim().toUpperCase(),
      referral_link: referral.referral_link || '',
      clicks: Number(referral.clicks) || 0,
      joins: Number(referral.joins) || 0,
      orders: Number(referral.orders) || 0,
      revenue_usd: Number(referral.revenue_usd) || 0,
      revenue_idr: Number(referral.revenue_idr) || 0,
      is_active: referral.is_active !== undefined ? referral.is_active : true,
    ...
    if (this.hasSupabase) {
      try {
        await this.supabase.from('referral_links').upsert(payload, { onConflict: 'referral_id' });
      } catch (e) {
        console.warn('[Supabase saveReferralLink note]:', e.message);
      }
    }
    try {
      await setDoc(doc(this.db, 'referral_links', referralId), payload, { merge: true });
    } catch (e) {}

    try {
      saveLocalReferralLink(payload);
    } catch (e) {}

    return payload;
  }

  async deleteReferralLink(referralId) {
    if (this.hasSupabase) {
      try {
        await this.supabase.from('referral_links').delete().eq('referral_id', referralId);
      } catch (e) {}
    }
    try {
      await deleteDoc(doc(this.db, 'referral_links', referralId));
    } catch (e) {}

    try {
      deleteLocalReferralLink(referralId);
    } catch (e) {}
  }

  async incrementReferralClick(referralId) {
    try {
      const link = await this.getReferralLink(referralId);
      if (!link) return null;
      return this.saveReferralLink({ ...link, clicks: (Number(link.clicks) || 0) + 1 });
    } catch (e) {
      return null;
    }
  }

  async incrementReferralJoin(referralId) {
    try {
      const link = await this.getReferralLink(referralId);
      if (!link) return null;
      return this.saveReferralLink({ ...link, joins: (Number(link.joins) || 0) + 1 });
    } catch (e) {
      return null;
    }
  }

  async incrementReferralOrder(referralId, amountUsd = 0, amountIdr = 0) {
    try {
      const link = await this.getReferralLink(referralId);
      if (!link) return null;
      return this.saveReferralLink({
        ...link,
        orders: (Number(link.orders) || 0) + 1,
        revenue_usd: (Number(link.revenue_usd) || 0) + amountUsd,
        revenue_idr: (Number(link.revenue_idr) || 0) + amountIdr
      });
    } catch (e) {
      return null;
    }
  ...
  async saveReferralClick(click) {
    const clickId = click.click_id || `clk_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
    const payload = {
      ...click,
      click_id: clickId,
      telegram_id: String(click.telegram_id || ''),
      clicked_at: click.clicked_at || new Date().toISOString(),
      joined: click.joined || false,
      joined_at: click.joined_at || null,
    ...
    if (this.hasSupabase) {
      try {
        await this.supabase.from('referral_clicks').insert(payload);
      } catch (e) {
        console.warn('[Supabase saveReferralClick note]:', e.message);
      }
    }
    try {
      await setDoc(doc(this.db, 'referral_clicks', clickId), payload, { merge: true });
    } catch (e) {}
    return payload;
  }

  async getReferralClicks(referralId = null, limitCount = 100) {
    let supabaseQuery = null;
    if (this.hasSupabase) {
      supabaseQuery = this.supabase.from('referral_clicks').select('*').order('clicked_at', { ascending: false }).limit(limitCount);
      if (referralId) supabaseQuery = supabaseQuery.eq('referral_id', referralId);
    }
    return this.queryWithFallback(supabaseQuery, async () => {
      let q = collection(this.db, 'referral_clicks');
      if (referralId) {
        q = query(q, where('referral_id', '==', referralId));
      }
      const snap = await getDocs(q);
      return snap.docs.map(d => d.data()).sort((a, b) => new Date(b.clicked_at).getTime() - new Date(a.clicked_at).getTime()).slice(0, limitCount);
    });
  }

  // --- Broadcast History ---
  async getBroadcastHistory(limitCount = 50) {
    let supabaseQuery = null;
    if (this.hasSupabase) {
      supabaseQuery = this.supabase.from('broadcast_history').select('*').order('created_at', { ascending: false }).limit(limitCount);
    }
    return this.queryWithFallback(supabaseQuery, async () => {
      try {
        const snap = await getDocs(query(collection(this.db, 'broadcast_history'), limitConstraint(limitCount)));
        if (!snap.empty) {
          return snap.docs.map(d => d.data()).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
        }
      } catch (e) {}
      ...
      try {
        return getLocalBroadcastHistory().slice(0, limitCount);
      } catch (e) {
        return [];
      }
    });
  ...
  async getBroadcastHistoryById(broadcastId) {
    if (!broadcastId) return null;
    if (this.hasSupabase) {
      try {
        const { data, error } = await this.supabase.from('broadcast_history').select('*').eq('broadcast_id', broadcastId).single();
        if (!error && data) return data;
      } catch (e) {}
    }
    try {
      const snap = await getDoc(doc(this.db, 'broadcast_history', broadcastId));
      if (snap.exists()) return snap.data();
    } catch (e) {}

    try {
      const locals = getLocalBroadcastHistory();
      return locals.find(b => (b.broadcast_id || b.id) === broadcastId) || null;
    } catch (e) {
      return null;
    }
  }

  async saveBroadcastHistory(broadcast) {
    const broadcastId = broadcast.broadcast_id || `bch_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
    const payload = {
      ...broadcast,
      broadcast_id: broadcastId,
      total_recipients: Number(broadcast.total_recipients) || 0,
      sent_count: Number(broadcast.sent_count) || 0,
      failed_count: Number(broadcast.failed_count) || 0,
      status: broadcast.status || 'pending',
      started_at: broadcast.started_at || null,
      finished_at: broadcast.finished_at || null,
      error: broadcast.error || null,
      created_by: broadcast.created_by || 'system',
      created_at: broadcast.created_at || new Date().toISOString()
    };

    if (this.hasSupabase) {
      try {
        await this.supabase.from('broadcast_history').upsert(payload, { onConflict: 'broadcast_id' });
      } catch (e) {
        console.warn('[Supabase saveBroadcastHistory note]:', e.message);
      }
    }
    try {
      await setDoc(doc(this.db, 'broadcast_history', broadcastId), payload, { merge: true });
    } catch (e) {}

    try {
      saveLocalBroadcastHistory(payload);
    } catch (e) {}

    return payload;
  }

  async deleteBroadcastHistory(broadcastId) {
    if (this.hasSupabase) {
      try {
        await this.supabase.from('broadcast_history').delete().eq('broadcast_id', broadcastId);
      } catch (e) {}
    }
    try {
      await deleteDoc(doc(this.db, 'broadcast_history', broadcastId));
    } catch (e) {}

    try {
      deleteLocalBroadcastHistory(broadcastId);
    } catch (e) {}
  }

  // --- Super Admins (Root Admin Management) ---
  async getSuperAdmins() {
    let supabaseQuery = null;
    if (this.hasSupabase) {
      supabaseQuery = this.supabase.from('super_admins').select('*').order('created_at', { ascending: false });
    }
    return this.queryWithFallback(supabaseQuery, async () => {
      const snap = await getDocs(collection(this.db, 'super_admins'));
      return snap.docs.map(d => d.data());
    });
  }
  ...
  async getSuperAdmin(adminId) {
    ...
    if (this.hasSupabase) {
      try {
        const { data, error } = await this.supabase.from('super_admins').select('*').eq('admin_id', adminId).single();
        if (!error && data) return data;
      } catch (e) {}
    }
    try {
      const snap = await getDoc(doc(this.db, 'super_admins', adminId));
      if (snap.exists()) return snap.data();
    } catch (e) {}
    return null;
  }

  async isRootAdmin(adminId) {
    const admin = await this.getSuperAdmin(adminId);
    return admin?.is_root === true;
  }

  async saveSuperAdmin(admin) {
    const adminId = admin.admin_id || `sadm_${Date.now()}`;
    const payload = {
      ...admin,
      admin_id: adminId,
      ...
      permissions: admin.permissions || { manage_admins: true, manage_channels: true, manage_products: true, manage_settings: true, view_all: true },
      is_root: admin.is_root || false,
    ...
    if (this.hasSupabase) {
      try {
        await this.supabase.from('super_admins').upsert(payload, { onConflict: 'admin_id' });
      } catch (e) {
        console.warn('[Supabase saveSuperAdmin note]:', e.message);
      }
    }
    try {
      await setDoc(doc(this.db, 'super_admins', adminId), payload, { merge: true });
    } catch (e) {}
    return payload;
  }

  async deleteSuperAdmin(adminId) {
    const allAdmins = await this.getSuperAdmins();
    if (allAdmins.length <= 1) {
      throw new Error('Tidak bisa menghapus super admin terakhir');
    }
    if (this.hasSupabase) {
      try {
        await this.supabase.from('super_admins').delete().eq('admin_id', adminId);
      } catch (e) {}
    }
    try {
      await deleteDoc(doc(this.db, 'super_admins', adminId));
    } catch (e) {}
  }
  ...
  async updateSuperAdminLastLogin(adminId) {
    ...
    if (this.hasSupabase) {
      try {
        await this.supabase.from('super_admins').update({ last_login: new Date().toISOString() }).eq('admin_id', adminId);
      } catch (e) {}
    }
    try {
      await setDoc(doc(this.db, 'super_admins', adminId), { last_login: new Date().toISOString() }, { merge: true });
    } catch (e) {}
  }
}
