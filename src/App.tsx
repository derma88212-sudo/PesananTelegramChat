...
export function App() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'orders' | 'products' | 'payment_methods' | 'coupons' | 'wallets' | 'channels' | 'bots' | 'database' | 'settings' | 'admins'>('dashboard');
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        ...
        if (settings.auto_purge_cancelled) {
          ...
          if (hasCancelled) {
            fetch('/api/orders/purge-cancelled', { method: 'POST' }).catch(() => {});
          }
        }
  ...
  const handleApproveOrder = async (orderId: string) => {
    ...
    if (data.success) {
      ...
      dialogAlert(data.message || 'Gagal menyetujui pesanan', 'Gagal Setujui', 'error');
    }
  };
  ...
  const handleCancelOrder = async (orderId: string) => {
    ...
    if (data.success) {
      ...
      dialogAlert(data.message || 'Gagal membatalkan pesanan', 'Gagal Batal', 'error');
    }
  };
  ...
  const handleResetCompletedOrders = async (mode: 'to_pending' | 'purge' = 'to_pending') => {
    ...
    const data = await res.json();
    if (data.success) {
      showToast(`Berhasil memproses ${data.count || 0} pesanan selesai`);
      refreshAll();
    } else {
      dialogAlert(data.message || 'Gagal mereset pesanan selesai', 'Gagal Reset', 'error');
    }
  };
  ...
  const handleRejectOrder = async (orderId: string) => {
    ...
    if (data.success) {
      ...
      dialogAlert(data.message || 'Gagal reject pesanan', 'Gagal Tolak', 'error');
    }
  };
  ...
  const handleDeleteOrder = async (orderId: string) => {
    ...
    const data = await res.json();
    if (data.success) {
      showToast(`Pesanan #${orderId} dihapus dari database`);
      refreshAll();
    } else {
      dialogAlert(data.message || 'Gagal menghapus pesanan', 'Gagal Hapus', 'error');
    }
  };
  ...
  const handlePurgeCancelledOrders = async () => {
    ...
    const data = await res.json();
    if (data.success) {
      showToast(`Berhasil menghapus ${data.purged_count} pesanan yang dibatalkan`);
      refreshAll();
    }
  };
  ...
  const handleSavePaymentMethod = async (method: Partial<PaymentMethod>) => {
    ...
    const data = await res.json();
    if (!data.success) throw new Error(data.message || 'Gagal menyimpan metode pembayaran');
    fetchPaymentMethods();
  };

  const handleDeletePaymentMethod = async (methodId: string) => {
    const res = await fetch(`/api/payment-methods/${methodId}`, { method: 'DELETE' });
    const data = await res.json();
    if (!data.success) throw new Error(data.message || 'Gagal menghapus metode pembayaran');
    fetchPaymentMethods();
  };

  const handleTogglePaymentMethod = async (methodId: string) => {
    const res = await fetch(`/api/payment-methods/${methodId}/toggle`, { method: 'PATCH' });
    const data = await res.json();
    if (data.success) {
      fetchPaymentMethods();
  ...
  const handleSaveChannel = async (channel: Partial<TelegramChannel>) => {
    ...
    const data = await res.json();
    if (!data.success) throw new Error(data.message || 'Gagal menyimpan channel');
    fetchChannels();
  };
  ...
  const handleDeleteChannel = async (channelId: string) => {
    ...
    const data = await res.json();
    if (!data.success) throw new Error(data.message || 'Gagal menghapus channel');
    fetchChannels();
  };

  const handleToggleChannel = async (channelId: string) => {
    const res = await fetch(`/api/channels/${channelId}/toggle`, { method: 'PATCH' });
    const data = await res.json();
    if (data.success) {
      fetchChannels();
  ...
  const handleAddBulkStocks = async (productId: string, accountsText: string): Promise<number> => {
    ...
    throw new Error(data.message || 'Gagal menambahkan stok');
  };
  ...
  const handleAddBot = async (name: string, token: string) => {
    ...
    if (data.success) {
      ...
      dialogAlert(data.message || 'Gagal menambah bot', 'Gagal Tambah Bot', 'error');
    }
  };

  const handleLaunchBot = async (id: string) => {
    const res = await fetch(`/api/bots/${id}/launch`, { method: 'POST' });
    const data = await res.json();
    if (data.success) {
      ...
      dialogAlert(data.message || 'Gagal menjalankan bot', 'Gagal Menjalankan Bot', 'error');
    }
  };
  ...
  const handleSaveSettings = async (newSettings: Partial<StoreSettings>) => {
    ...
    if (data.success) {
      ...
      dialogAlert(data.message || 'Gagal menyimpan pengaturan', 'Gagal Simpan', 'error');
    }
  };
