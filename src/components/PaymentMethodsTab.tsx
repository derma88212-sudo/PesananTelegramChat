...
export const PaymentMethodsTab: React.FC<PaymentMethodsTabProps> = ({
  ...
  const openAddModal = () => {
    ...
    setFormData({
      ...
      account_name: '',
      qr_image_url: '',
      instructions: 'Transfer nominal tepat sesuai invoice. Unggah bukti transfer setelah berhasil.',
      scope: 'ALL',
      is_active: true
    });
    setIsModalOpen(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await saveHandler({
        ...(editingMethod ? { method_id: editingMethod.method_id } : {}),
        ...formData
      });
  ...
  const handleDelete = async (method: PaymentMethod) => {
    ...
    try {
      await deleteHandler(method.method_id);
