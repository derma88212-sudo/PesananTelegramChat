...
CREATE TABLE IF NOT EXISTS settings (
  ...
  notify_on_txid BOOLEAN DEFAULT TRUE,
  notify_on_cancel BOOLEAN DEFAULT TRUE,
  notify_on_low_stock BOOLEAN DEFAULT TRUE,
  notify_on_broadcast BOOLEAN DEFAULT TRUE,
  -- Custom Notification Templates (JSON)
...
CREATE TABLE IF NOT EXISTS products (
  ...
  description TEXT,
  product_url TEXT, -- URL untuk produk digital (link download, akses, dll)
...
CREATE TABLE IF NOT EXISTS channels (
  ...
  referral_code TEXT UNIQUE, -- Kode referral unik
  referral_link TEXT, -- Link referral lengkap
  referrer_count INT DEFAULT 0, -- Jumlah yang join via referral
  total_referral_revenue_usd NUMERIC(12, 2) DEFAULT 0.00, -- Total pendapatan dari referral (USD)
  total_referral_revenue_idr NUMERIC(15, 2) DEFAULT 0.00 -- Total pendapatan dari referral (IDR)
);
...
-- ==============================================================================
-- NEW TABLES: Referral/Promo Links, Broadcast History, Super Admin Management
-- ==============================================================================

-- 16. Tabel Referral Links (Tautan Promosi Khusus per Channel/Bot)
CREATE TABLE IF NOT EXISTS referral_links (
  ...
  revenue_usd NUMERIC(12, 2) DEFAULT 0.00,
...
CREATE INDEX IF NOT EXISTS idx_referral_links_channel ON referral_links(channel_id);
CREATE INDEX IF NOT EXISTS idx_referral_links_code ON referral_links(referral_code);
CREATE INDEX IF NOT EXISTS idx_referral_links_bot ON referral_links(bot_token_id);

-- 17. Tabel Referral Clicks (Tracking klik & join via referral)
...
CREATE INDEX IF NOT EXISTS idx_referral_clicks_referral ON referral_clicks(referral_id);
CREATE INDEX IF NOT EXISTS idx_referral_clicks_telegram ON referral_clicks(telegram_id);
CREATE INDEX IF NOT EXISTS idx_referral_clicks_joined ON referral_clicks(joined);

-- 18. Tabel Broadcast History (Riwayat Broadcast Massal)
...
CREATE INDEX IF NOT EXISTS idx_broadcast_history_created ON broadcast_history(created_at);

-- 19. Tabel Super Admins (Manajemen Root Admin - hanya superadmin yang bisa CRUD)
CREATE TABLE IF NOT EXISTS super_admins (
  admin_id TEXT PRIMARY KEY REFERENCES admins(admin_id) ON DELETE CASCADE,
  username TEXT NOT NULL UNIQUE,
  password_hash TEXT NOT NULL,
  role TEXT DEFAULT 'superadmin',
  permissions JSONB DEFAULT '{"manage_admins": true, "manage_channels": true, "manage_products": true, "manage_settings": true, "view_all": true}'::jsonb,
  is_root BOOLEAN DEFAULT FALSE, -- Hanya 1 root admin yang tidak bisa dihapus/diubah oleh siapapun
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  last_login TIMESTAMPTZ
);

-- Insert Default Root Super Admin (tidak bisa dihapus)
INSERT INTO super_admins (admin_id, username, password_hash, role, is_root, permissions)
VALUES ('root_admin', 'root@admin.com', 'kucing123', 'superadmin', TRUE, '{"manage_admins": true, "manage_channels": true, "manage_products": true, "manage_settings": true, "view_all": true, "delete_root": false}'::jsonb)
ON CONFLICT (admin_id) DO NOTHING;
...
DO $$
BEGIN
  CREATE POLICY "Allow all on referral_links" ON referral_links FOR ALL USING (true) WITH CHECK (true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$
BEGIN
  CREATE POLICY "Allow all on referral_clicks" ON referral_clicks FOR ALL USING (true) WITH CHECK (true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$
BEGIN
  CREATE POLICY "Allow all on broadcast_history" ON broadcast_history FOR ALL USING (true) WITH CHECK (true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$
BEGIN
  CREATE POLICY "Allow all on super_admins" ON super_admins FOR ALL USING (true) WITH CHECK (true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
